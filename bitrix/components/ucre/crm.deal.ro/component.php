...
if ($mainDeal["CATEGORY_ID"] == 0 || $mainDeal["CATEGORY_ID"] == 4){
    ...
    $objectProperties = $mainObject->GetProperties();
		...
		if ($arResult['CITY'] == "Оренбург" ||$arResult['CITY'] == "Оренбург г") $arResult['LOCALITY'] = ($objectProperties['LOCALITY']['VALUE'])?$objectProperties['LOCALITY']['VALUE']:'<span style="color: red">нет данных</span>';
		else $arResult['LOCALITY'] = ($objectProperties['LOCALITY']['VALUE'])?$objectProperties['LOCALITY']['VALUE']:'<span style="grey: red">отсутствует</span>';
		$arResult['KAD_NUMBER'] = ($objectProperties['KAD_NUMBER']['VALUE'])?$objectProperties['KAD_NUMBER']['VALUE']:'<span style="color: red">нет данных</span>';
    ...
    $arResult['LINK'] = ($objectProperties['LINK']['VALUE'])?'<a href="'.$objectProperties['LINK']['VALUE'].'" target="_blank">ссылка на сайт</a>':'<span style="color: red">не выгружался на сайт</span>';
    switch ($objectProperties['TYPE']['VALUE']) {
      ...
      case 'дом':
			case 'таунхаус':
				...
				$arResult['TYPE'] = $rooms.$objectProperties['TYPE']['VALUE'];
				$arResult['TOTAL_AREA'] = ($objectProperties['TOTAL_AREA']['VALUE'])?$objectProperties['TOTAL_AREA']['VALUE']." м<sup>2</sup>":'<span style="color: red">нет данных</span>';
				$arResult['PLOT_AREA'] = ($objectProperties['PLOT_AREA']['VALUE'])?$objectProperties['PLOT_AREA']['VALUE']." сот.":'<span style="color: red">нет данных</span>';
				$arResult['FLOORALL'] = ($objectProperties['FLOORALL']['VALUE'])?$objectProperties['FLOORALL']['VALUE']:'<span style="color: red">нет данных</span>';
