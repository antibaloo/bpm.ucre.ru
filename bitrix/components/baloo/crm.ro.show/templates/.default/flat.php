...
?>
<?
$idro = $arResult['DATA']['ID'];
if (in_array($USER->GetID(), array(24))){
	echo "<h1>".$arResult['DATA']['NAME']."</h1>";//"ONCLICK"=>"jsUtils.Redirect(arguments, 'https://bpm.ucre.ru/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=42&type=CRM_PRODUCT_CATALOG&ID=".$arResult['DATA']['ID']."&lang=ru&find_section_section=-1&WF=Y')", "DEFAULT"=>true)
$APPLICATION->IncludeComponent(
	...
	array("id"=>"PROPERTY_321", "name"=>"Цена", "type"=>"label","value" => number_format($arResult['DATA']['PROPERTY_321'], 2, '.', ' ').' руб.'),
			...
			array("id"=>"tab2", "name"=>"Адрес", "icon"=>"", "fields"=>array(
				...
				array("id"=>"PROPERTY_219", "name"=>"Подъезд №", "type" => "label"),
				array("id"=>"PROPERTY_221", "name"=>"Этаж №", "type" => "label"),
				array("id"=>"PROPERTY_220", "name"=>"Квартира №", "type" => "label"),
				array("id"=>"section1", "type"=>"section"),
			)),
			array("id"=>"tab3", "name"=>"Информация о здании", "icon"=>"", "fields"=>array(
				array("id" => "PROPERTY_222", "name" => "Этажность", "type" => "label"),
				array("id"=>"section1", "type"=>"section"),
			)),
			array("id"=>"tab4", "name"=>"Информация о квартире", "icon"=>"", "fields"=>array(
				array("id" => "PROPERTY_229", "name" => "Количество комнат", "type" => "label", "value"=>intval ($arResult['DATA']['PROPERTY_229'])),
				array("id" => "PROPERTY_224", "name" => "Общая площадь", "type" => "label", "value" => number_format($arResult['DATA']['PROPERTY_224'], 2, ',', '').' кв.м.'),
				array("id" => "PROPERTY_225", "name" => "Жилая площадь", "type" => "label", "value" => number_format($arResult['DATA']['PROPERTY_225'], 2, ',', '').' кв.м.'),
				array("id" => "PROPERTY_226", "name" => "Площадь кухни", "type" => "label", "value" => number_format($arResult['DATA']['PROPERTY_226'], 2, ',', '').' кв.м.'),
				array("id"=>"section1", "type"=>"section"),
			)),
		...
		"BUTTONS"=>array("back_url"=>".", 
										 "custom_html"=>"<input type='button' value='Вернуться' name='back' onclick='window.location=&quot;https://bpm.ucre.ru/crm/ro/&quot;' title='Вернуться'>",
...
$APPLICATION->IncludeComponent(
	...
	array("id"=>"PROPERTY_321", "name"=>"Цена", "type"=>"label","value" => number_format($arResult['DATA']['PROPERTY_321'], 2, '.', ' ').' руб.'),
		...
		"BUTTONS"=>array("back_url"=>".", "custom_html"=>"<input type='button' value='Вернуться' name='back' onclick='window.location=&quot;https://bpm.ucre.ru/crm/ro/&quot;' title='Вернуться'>", "standard_buttons"=>false),
		"DATA"=>$arResult["DATA"],
	),
	$component
);
