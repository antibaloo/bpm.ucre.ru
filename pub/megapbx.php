...
if ($_POST['crm_token'] == $megapbx->crm_key){
  ...
  if ($_POST['cmd'] == 'event' && $_POST['type'] == 'OUTGOING'){
    ...
    if ($phone_res['FOUND'] == 'N'){
      $oLead = new CCrmLead;
      $arFields = array(
        "TITLE" => "Лид по исходящему звонку на  номер +7(".substr($_POST['phone'],1,3).")".substr($_POST['phone'],4,3)."-".substr($_POST['phone'],7,2)."-".substr($_POST['phone'],9)." на ВАТС Мегафон",
        "NAME" => "неизвестно",
        "COMMENTS" => "",
        "SOURCE_ID" => "CALL",
        "OPPORTUNITY" =>0,
      ...
      $arMessageFields = array(
        ...
        //"NOTIFY_TAG" => "CRM|LEAD|NEW|MEGAPBX",
        // текст уведомления на сайте (доступен html и бб-коды)
        "NOTIFY_MESSAGE" => "[b]Создан новый лид по исходящему звонку[/b] на номер +7(".substr($_POST['phone'],1,3).")".substr($_POST['phone'],4,3)."-".substr($_POST['phone'],7,2)."-".substr($_POST['phone'],9)." на ВАТС Мегафон. <a href='/crm/lead/show/".$LeadId."/' target='_blank'>Перейти к лиду</a>",
      );
      CIMNotify::Add($arMessageFields);
      $arMessageFields = array(
        // получатель
  ...
  if ($_POST['cmd'] == 'history' && $_POST['type'] == 'out'){
    ...
    if ($phone_res['FOUND']['Y'] && ($phone_res['LEAD']['ID']>0 || $phone_res['CONTACT']['ID']>0)){
      ...
      $arFields = array(
        ...
        'OWNER_TYPE_ID' => CCrmOwnerType::ResolveID($entity_type),
        'TYPE_ID' =>  CCrmActivityType::Call,
        'SUBJECT' => 'Исходящий '.$outRes.' звонок на номер +7('.substr($_POST['phone'],1,3).")".substr($_POST['phone'],4,3)."-".substr($_POST['phone'],7,2)."-".substr($_POST['phone'],9),
        'START_TIME' => date("d.m.Y H:i:s",strtotime($begintime['TIME'])),
        'END_TIME' => date("d.m.Y H:i:s",strtotime($begintime['TIME'])+$_POST['duration']),
        'COMPLETED' => ($_POST['status'] == 'Success')?'Y':'N',
        'RESPONSIBLE_ID' => $assignedById,
