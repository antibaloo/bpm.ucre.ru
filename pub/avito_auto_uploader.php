...
if ($_GET['count']>0 && in_array($_SERVER['GEOIP_ADDR'], array('178.21.8.184'))){
  //print_r($_SERVER);
  //echo "На выгрузку ".$_GET['count']." заявок, до ".date("Y-m-d", strtotime("+30 days"))."<br>";
  ...
  while($aRes = $db_res->Fetch()){
    ...
    //echo "ID заявки ".$aRes['ID'].",  ID объекта ".$aRes['ELEMENT_ID'].", ".$type[$aRes['PROPERTY_210']]."<br>";
  }
